...
function embed() {
  ...
  const tabSize = target.searchParams.get("ts") || 4;
...
`);
  ...
  Promise.all(showLineNumbers ? [fetchFile, loadHLJS, loadHLJSNum] : [fetchFile, loadHLJS]).then((result) => {
    ...
    embedCodeToTarget(targetDiv, result[0], showBorder, showLineNumbers, isDarkStyle, target.href, rawFileURL, fileExtension, startLine, endLine, tabSize, sourceURL.origin);
  }).catch((error) => {
    const errorMsg = `Failed to process ${rawFileURL}
${error}`;
    const targetDiv = document.getElementById(containerId);
    embedCodeToTarget(targetDiv, errorMsg, showBorder, showLineNumbers, isDarkStyle, target.href, rawFileURL, 'plaintext', -1, -1, tabSize, sourceURL.origin);
  });
}
...
function embedCodeToTarget(targetDiv, codeText, showBorder, showLineNumbers, isDarkStyle, fileURL, rawFileURL, lang, startLine, endLine, tabSize, serviceProvider) {
  targetDiv.innerHTML = "";
